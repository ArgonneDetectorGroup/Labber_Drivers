...
class Driver(InstrumentDriver.InstrumentWorker):
    ...
    def performSetValue(self, quant, value, sweepRate=0.0, options={}):
        ...
        if quant.name == 'Heat Switch':
            ...
            monitorToggle.CreateDIChan('ADR_DIO/port0/line0:1','', mx.DAQmx_Val_ChanPerLine)

            isToggling = True
            startTime = time.time()
            while isToggling == True:
                monitorToggle.ReadDigitalLines(1, 0, mx.DAQmx_Val_GroupByScanNumber, monVal, 2, mx.byref(sampsPer), mx.byref(numBytesPer), None)
                if all(monVal):
                    isToggling = False
                self.wait(0.2)
            ...
            self.readValueFromOther('Touch 4K-1K')
            self.readValueFromOther('Touch 4K-50mK')
            self.readValueFromOther('Touch 1K-50mK')
    ...
    def performGetValue(self, quant, options={}):
        ...
        if quant.name == 'Touch 4K-1K':
            channel = 'ADR_DIO/port0/line2'
        elif quant.name == 'Touch 4K-50mK':
            channel = 'ADR_DIO/port0/line3'
        elif quant.name == 'Touch 1K-50mK':
        ...
        readChan.ReadDigitalLines(1, 0, mx.DAQmx_Val_GroupByScanNumber, retVal, 1, mx.byref(sampsPer), mx.byref(numBytesPer), None)
